...
class LevelsControllerTest < ActionController::TestCase
  ...
  test "add_skill adds a skill" do
    ...
    assert level.skill_keys.include?(skill.key)
  end

  test "add multiple skills" do
    level = create :level
    assert level.skills.empty?
    skill_1 = create :skill
    post :add_skill, params: {id: level.id, levelId: level.id, skillId: skill_1.id}
    assert_response :success
    level.reload
    assert_equal [skill_1], level.skills
    assert level.skill_keys.include?(skill_1.key)
    skill_2 = create :skill
    post :add_skill, params: {id: level.id, levelId: level.id, skillId: skill_2.id}
    assert_response :success
    level.reload
  ...
  test "remove_skill removes a skill" do
    ...
    assert level.skill_keys.include?(skill.key)
    assert level.skill_keys == [skill.key].to_json
