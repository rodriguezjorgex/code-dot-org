module AiSystemPrompts::EvaluateSystemPromptHelper
  def self.get_system_prompt(level, unit, evaluation_type)
    ...
    single_student_evaluation_structure = <<~TEXT
      ...
      evaluationCriteria is should just be a copy of #{evaluation_summary}.
  ...
  def self.get_skills_based_evaluation_criteria
    {
      # Lesson 3, Level 2
      22595 => {
  ...
  def self.get_skills_based_evaluation_criteria_for_level(level_id)
    get_skills_based_evaluation_criteria[level_id]
  end

  def self.get_evaluation_criteria_summary(level)
    skills_based_criteria = get_skills_based_evaluation_criteria_for_level(level.id)
    summary = "Evaluated for completion based on a level instructions."
    if !skills_based_criteria && level.upper_grades_programming_level?
      summary << "/n Evaluated for general code quality."
    elsif skills_based_criteria
      programming_concept_skills = skills_based_criteria[:programming_concept_skills]
  ...
  def self.get_evaluation_criteria(level)
    completion_criteria = "Did the student complete all of the requirements in the instructions? The instructions are: #{level.properties["long_instructions"]}"
    evaluation_criteria = [completion_criteria]

    skills_based_criteria = get_skills_based_evaluation_criteria_for_level(level.id)

    if skills_based_criteria
      skills_based_criteria[:programming_concept_skills].each do |criterion|
