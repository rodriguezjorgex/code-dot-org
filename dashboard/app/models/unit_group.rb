    ...
    ActiveRecord::Base.connected_to(role: :reading) do
      {
        ...
        teacher_resources: resources.filter {|r| r.embeddability_type != SharedConstants::RESOURCE_EMBEDDABILITY_OPTIONS[:EMBED_ONLY][:value]}.sort_by(&:name).map(&:summarize_for_resources_dropdown),
        student_resources: student_resources.filter {|r| r.embeddability_type != SharedConstants::RESOURCE_EMBEDDABILITY_OPTIONS[:EMBED_ONLY][:value]}.sort_by(&:name).map(&:summarize_for_resources_dropdown),
